...
class MyPage extends StatelessWidget {
  ...
  Future<Map<String, dynamic>> getMyInfo(BuildContext context) async {
    ...
    final url = Uri.parse('https://your-api.com/users/me');
  ...
  // 내 점수 가져오기
  Future<int> getMyScore(BuildContext context) async {
    final token = context.read<TokenProvider>().token;
    final url = Uri.parse('https://wiset-deepfake-server.onrender.com/users/me/classRank');
    ...
    if (response.statusCode == 200) {
      ...
      return data['myRank']['score'];
    } else {
      throw Exception('점수를 불러오지 못했습니다.');
    }
  }
    ...
    return Scaffold(
      ...
      body: FutureBuilder<Map<String, dynamic>>(
        ...
        builder: (context, snapshot) {
          ...
          final school = userInfo['schoolCode'] ?? '학교 없음';

          return FutureBuilder<int>(
            future: getMyScore(context),
            builder: (context, scoreSnap) {
              final scoreWidget = scoreSnap.connectionState == ConnectionState.waiting
                  ? const Text('불러오는 중...', style: TextStyle(fontSize: 22, color: Colors.white))
                  : scoreSnap.hasError
                  ? const Text('점수 오류', style: TextStyle(fontSize: 22, color: Colors.white))
                  : Text('${scoreSnap.data} 점',
                  ...
                  child: Column(
                    children: [
                      ...
                      Container(
                        ...
                        child: Column(
                          children: [
                            Text(school, style: const TextStyle(fontSize: 16, color: Colors.white)),
